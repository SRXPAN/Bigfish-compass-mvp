 ...
 */
function getLocalized(jsonField, lang, fallback) {
    if (jsonField && typeof jsonField === 'object' && !Array.isArray(jsonField)) {
        if (jsonField[lang])
            return jsonField[lang];
        if (jsonField['EN'])
            return jsonField['EN'];
    }
    return fallback;
 ...
 */
export async function getQuizWithToken(quizId, userId, lang) {
 ...
 */
export async function submitQuizAttempt(quizId, userId, token, answers, lang) {
    // Verify quiz token
    let quizPayload;
    try {
        quizPayload = jwt.verify(token, getJwtSecret());
    }
    catch (e) {
    ...
    for (const q of quiz.questions) {
        ...
        if (lang && q.explanationJson) {
            explanationMap[q.id] = getLocalized(q.explanationJson, lang, q.explanation || '');
        }
 ...
 */
export async function getUserQuizHistory(userId, options) {
