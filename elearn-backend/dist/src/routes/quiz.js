...
// GET /api/quiz/:id
router.get('/:id', requireAuth, validateResource(quizSchemas.idParam, 'params'), asyncHandler(async (req, res) => {
...
validateResource(quizSchemas.idParam, 'params'), validateResource(quizSchemas.submitQuiz, 'body'), asyncHandler(async (req, res) => {
    const id = getParam(req.params.id);
    const { token, answers, lang } = req.body;
    try {
        const result = await submitQuizAttempt(id, req.user.id, token, answers, lang || 'EN');
...
router.get('/user/history', requireAuth, validateResource(historyQuerySchema, 'query'), asyncHandler(async (req, res) => {
    const parsed = req.queryParsed;
