...
import { generateFileKey, getPresignedUploadUrl, getPresignedDownloadUrl, getPublicUrl, deleteFile, validateFile, } from '../services/storage.service';
...
// --- ROUTES ---
router.post('/presign-upload', requireAuth, uploadLimiter, validateResource(uploadPresignSchema, 'body'), asyncHandler(async (req, res) => {
    const { filename, mimeType, size, category = 'attachments' } = req.body;
    const validCategories = ['avatars', 'materials', 'attachments'];
...
router.post('/confirm', requireAuth, validateResource(z.object({ fileId: z.string().cuid() }), 'body'), // Added validation
...
router.get('/:id', requireAuth, validateResource(fileIdSchema, 'params'), // Added validation
asyncHandler(async (req, res) => {
...
router.delete('/:id', requireAuth, validateResource(fileIdSchema, 'params'), // Added validation
asyncHandler(async (req, res) => {
    const id = getParam(req.params.id);
    const file = await prisma.file.findUnique({ where: { id } });
    if (!file)
