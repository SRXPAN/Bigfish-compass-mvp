...
router.get('/stats', requireRole(['ADMIN', 'EDITOR']), async (req, res) => {
    ...
    try {
        const [totalUsers, usersByRole, totalTopics, totalMaterials, totalQuizzes, totalQuestions, totalFiles, recentActivity,] = await Promise.all([
        ...
        const activityByDate = recentActivity.reduce((acc, curr) => {
...
router.get('/content/topics', async (req, res) => {
    try {
        ...
        logger.error('Get topics error', err);
 ...
 */
router.post('/content/topics/:id/quizzes', async (req, res) => {
    try {
 ...
 */
router.get('/content/quizzes/:id', async (req, res) => {
    try {
        ...
        logger.error('Get quiz error', err);
 ...
 */
router.put('/content/quizzes/:id', async (req, res) => {
    try {
        const quizId = getParam(req.params.id);
        ...
        logger.error('Update quiz error', err);
 ...
 */
router.delete('/content/quizzes/:id', async (req, res) => {
    try {
        const quizId = getParam(req.params.id);
        const quiz = await prisma.quiz.findUnique({
            where: { id: quizId },
        ...
        logger.error('Delete quiz error', err);
 ...
 */
router.post('/content/quizzes/:id/questions', async (req, res) => {
    try {
        const quizId = getParam(req.params.id);
 ...
 * Update a question
 */
router.put('/content/questions/:id', async (req, res) => {
    try {
 ...
 */
router.put('/content/options/:id', async (req, res) => {
    try {
        ...
        logger.error('Update option error', err);
 ...
 * Delete an option
 */
router.delete('/content/options/:id', async (req, res) => {
    try {
        const optionId = getParam(req.params.id);
        const existing = await prisma.option.findUnique({
            where: { id: optionId },
        ...
        logger.error('Delete option error', err);
